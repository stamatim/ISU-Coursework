...
class MiniNN:
  ...
  def predict(self, X_0):
    ...
    for W in self.Ws:
      ...
      Xs.append(X)
    self.Xs = Xs
    # it is safe because Python preserves variables used in for-loops
    self.oracle = X[1:]
  ...
  def get_deltas(self, target):
    ...
    # artificially prepend the delta on bias to match that in non-output layers.
    delta = numpy.concatenate(([0], delta))
